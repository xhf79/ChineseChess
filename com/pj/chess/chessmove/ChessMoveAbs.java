...
//		System.out.println("还原棋子后Zobrist："+TranspositionTable.boardZobrist);
	...
	*/
	public boolean legalMove(int play,MoveNode moveNode){
		...
		//原棋子不为已方棋子或目标棋子为已方棋子
		if((chessPlay[play] & srcChess)==0){//点击棋子不是自己的 2021-04-16
			return false;
		}
		if(destChess!=NOTHING && (chessPlay[play] & destChess)!=0){//吃掉的棋子，是自己的。 2021-04-16
			return false;
		}
		if(srcChess!=moveNode.srcChess || destChess!=moveNode.destChess){//原棋子或目标棋子不相符？这种情况会出现？2021-04-16
			return false;
		}
		int srcSite=moveNode.srcSite;
		int destSite=moveNode.destSite;
